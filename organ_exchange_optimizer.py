...
def find_organ_exchange(g):
    """
    Finds the maximum set of disjoint cycles for organ exchange optimization.
    ...
    Args:
        g (networkx.DiGraph): Directed compatibility graph
        
    Returns:
        tuple: (cycles, total_transplants)
            cycles (list): List of selected exchange cycles
            total_transplants (int): Total number of patients helped
    """
    ...
    prob = pulp.LpProblem("organ_exchange", pulp.LpMaximize)
...
def generate_random_graph(n, p):
    """
    Generate random directed graph with n nodes, edge probability p.
    
    Args:
        n (int): Number of nodes
        p (float): Probability of edge creation (0 to 1)
        
    Returns:
        networkx.DiGraph: Random directed graph
    """
    g = nx.DiGraph()
